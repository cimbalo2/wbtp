...
(*TODO carcare time da sostituire*)
let time f x = f x;;
(*let time f x =*)
...
primary_parser '(x1 + x2 + x3) * (1 + 2 + 3 * x + y)';;
*)
...
(*
string_of_exp primary_parser 'x + 3 * y';;
*)
...
let print_exp e = Format.print_string ("primary_parser '"^string_of_exp 0 e^"'");;
...
(*
primary_parser 'x + 3 * y';;
primary_parser '(x + 3) * y';;
primary_parser '1 + 2 + 3';;
primary_parser '((1 + 2) + 3) + 4';;
*)
...
(*
let fm = primary_parser 'p ==> q <=> r /\ s \/ (t <=> ~ ~u /\ v)';;
...
(*
atoms primary_parser 'p /\ q \/ s ==> ~p \/ (r <=> s)';;
*)
...
(*
print_truthtable primary_parser 'p /\ q ==> q /\ r';;

let fm = primary_parser 'p /\ q ==> q /\ r';;
...
(*
print_truthtable primary_parser '((p ==> q) ==> p) ==> p';;

print_truthtable primary_parser 'p /\ ~p';;
*)
...
tautology primary_parser 'p \/ ~p';;

tautology primary_parser 'p \/ q ==> p';;

tautology primary_parser 'p \/ q ==> q \/ (p <=> q)';;

tautology primary_parser '(p \/ q) /\ ~(p /\ q) ==> (~p <=> q)';;
...
(*
psubst (P"p" |=> primary_parser 'p /\ q') primary_parser 'p /\ q /\ p /\ q';;
*)
...
(*
tautology primary_parser '(p ==> q) \/ (q ==> p)';;

tautology primary_parser 'p \/ (q <=> r) <=> (p \/ q <=> p \/ r)';;
...
forall tautology
 [primary_parser 'true <=> false ==> false';
  primary_parser '~p <=> p ==> false';
  primary_parser 'p /\ q <=> (p ==> q ==> false) ==> false';
  primary_parser 'p \/ q <=> (p ==> false) ==> q';
  primary_parser '(p <=> q) <=> ((p ==> q) ==> (q ==> p) ==> false) ==> false'];;
*)
...
(*
psimplify primary_parser '(true ==> (x <=> false)) ==> ~(y \/ false /\ z)';;

psimplify primary_parser '((x ==> y) ==> true) \/ ~false';;
*)
...
(*
tautology primary_parser '(p ==> p') /\ (q ==> q') ==> (p /\ q ==> p' /\ q')';;
tautology primary_parser '(p ==> p') /\ (q ==> q') ==> (p \/ q ==> p' \/ q')';;
*)
...
(*
let fm = primary_parser '(p \/ q /\ r) /\ (~p \/ ~r)';;

dnf fm;;

print_truthtable fm;;

dnf primary_parser 'p /\ q /\ r /\ s /\ t /\ u \/ u /\ v';;
*)
...
(*
rawdnf primary_parser '(p \/ q /\ r) /\ (~p \/ ~r)';;
*)
...
(*
purednf primary_parser '(p \/ q /\ r) /\ (~p \/ ~r)';;
*)
...
let trigger =
  let [trig_and; trig_or; trig_imp; trig_iff] = map triggers
      [primary_parser 'p <=> q /\ r'; primary_parser 'p <=> q \/ r';
       primary_parser 'p <=> (q ==> r)'; primary_parser 'p <=> (q <=> r)']
  and p = primary_parser 'p' and q = primary_parser 'q' and r = primary_parser 'r'
...
(*
primary_parser '(forall x. x < 2 ==> 2 * x <= 3) \/ false';;
...
(*
primary_parser 'forall x y. exists z. x < z /\ y < z';;

primary_parser '~(forall x. P(x)) <=> exists y. ~P(y)';;
*)
...
(*
holds bool_interp undefined primary_parser 'forall x. (x = 0) \/ (x = 1)';;

holds (mod_interp 2) undefined primary_parser 'forall x. (x = 0) \/ (x = 1)';;

holds (mod_interp 3) undefined primary_parser 'forall x. (x = 0) \/ (x = 1)';;

let fm = primary_parser 'forall x. ~(x = 0) ==> exists y. x * y = 1';;

filter (fun n -> holds (mod_interp n) undefined fm) (1--45);;

holds (mod_interp 3) undefined primary_parser '(forall x. x = 0) ==> 1 = 0';;
holds (mod_interp 3) undefined primary_parser 'forall x. x = 0 ==> 1 = 0';;
*)
...
(*
subst ("y" |=> Var "x") primary_parser 'forall x. x = y';;

subst ("y" |=> Var "x") primary_parser 'forall x x'. x = y ==> x = x'';;
*)
(* ========================================================================= *)
...
(*
simplify primary_parser '(forall x y. P(x) \/ (P(y) /\ false)) ==> exists z. Q';;
*)
...
(*
skolemize primary_parser 'exists y. x < y ==> forall u. exists v. x * u < y * v';;
...
(*
gilmore primary_parser 'exists x. forall y. P(x) ==> P(y)';;

let sfm = skolemize(Not primary_parser 'exists x. forall y. P(x) ==> P(y)');;
...
(*
let p19 = compare
 primary_parser 'exists x. forall y z. (P(y) ==> Q(z)) ==> P(x) ==> Q(x)';;
...
let p39 = compare
 primary_parser '~(exists x. forall y. P(y,x) <=> ~P(y,y))';;

let p42 = compare
 primary_parser '~(exists y. forall x. P(x,y) <=> ~(exists z. P(x,z) /\ P(z,x)))';;
...
let p59 = compare
 primary_parser '(forall x. P(x) <=> ~P(f(x))) ==> (exists x. P(x) /\ ~P(f(x)))';;
...
let p8 = time splittab
 primary_parser '((p ==> q) ==> p) ==> p';;

let p9 = time splittab
 primary_parser '(p \/ q) /\ (~p \/ q) /\ (p \/ ~q) ==> ~(~q \/ ~q)';;

let p10 = time splittab
 primary_parser '(q ==> r) /\ (r ==> p /\ q) /\ (p ==> q /\ r) ==> (p <=> q)';;

let p11 = time splittab
 primary_parser 'p <=> p';;

let p12 = time splittab
 primary_parser '((p <=> q) <=> r) <=> (p <=> (q <=> r))';;

let p13 = time splittab
 primary_parser 'p \/ q /\ r <=> (p \/ q) /\ (p \/ r)';;

let p14 = time splittab
 primary_parser '(p <=> q) <=> (q \/ ~p) /\ (~q \/ p)';;

let p15 = time splittab
 primary_parser 'p ==> q <=> ~p \/ q';;

let p16 = time splittab
 primary_parser '(p ==> q) \/ (q ==> p)';;

let p17 = time splittab
 primary_parser 'p /\ (q ==> r) ==> s <=> (~p \/ q \/ s) /\ (~p \/ ~r \/ s)';;
...
let p18 = time splittab
 primary_parser 'exists y. forall x. P(y) ==> P(x)';;
...
let p22 = time splittab
 primary_parser '(forall x. P <=> Q(x)) ==> (P <=> (forall x. Q(x)))';;
...
let p35 = time splittab
 primary_parser 'exists x y. P(x,y) ==> (forall x y. P(x,y))';;
...
let barb = primary_parser '~(exists b. forall x. shaves(b,x) <=> ~shaves(x,x))';;
...
let p21 = time resolution
 primary_parser '(exists x. P ==> Q(x)) /\ (exists x. Q(x) ==> P) ==> (exists x. P <=> Q(x))';;
...
hornprove primary_parser '(p \/ q) /\ (~p \/ q) /\ (p \/ ~q) ==> ~(~q \/ ~q)';;
...
(*
tab primary_parser 'forall a. ~(P(a) /\ (forall y z. Q(y) \/ R(z)) /\ ~P(a))';;

tab primary_parser 'forall a. ~(P(a) /\ ~P(a) /\ (forall y z. Q(y) \/ R(z)))';;
...
(*
skolemizes [primary_parser 'exists x y. x + y = 2';
            primary_parser 'forall x. exists y. x + 1 = y'];;
*)
(* ========================================================================= *)
...
let equivalence_axioms =
  [primary_parser 'forall x. x = x'; primary_parser 'forall x y z. x = y /\ x = z ==> y = z'];;
...
let fm = equalitize
 primary_parser 'forall c. f(f(f(f(f(c))))) = c /\ f(f(f(c))) = c ==> f(c) = c';;
...
ccvalid primary_parser 'f(f(f(f(c)))) = c /\ f(f(c)) = c ==> f(c) = c';;
...
(*
rewrite [primary_parser '0 + x = x'; primary_parser 'S(x) + y = S(x + y)';
         primary_parser '0 * x = 0'; primary_parser 'S(x) * y = y + x * y']
...
(*
let eq = primary_parser 'f(f(x)) = g(x)' in critical_pairs eq eq;;
*)
...
(*
let eqs =
  [primary_parser '1 * x = x'; primary_parser 'i(x) * x = 1'; primary_parser '(x * y) * z = x * y * z'];;
...
skolemize primary_parser 'forall x z. exists w. forall y. z = x * y ==> w = y';;
*)
...
complete_and_simplify ["1"; "*"; "i"]
 [primary_parser '(x * y) * z = x * (y * z)';
  primary_parser '1 * x = x'; primary_parser 'x * 1 = x'; primary_parser 'x * x = 1'];;
...
(*
let eqs =  [primary_parser '(a * b) * (b * c) = b'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z'; primary_parser '1 * x = x'; primary_parser 'x * i(x) = 1'];;
...
let eqs =
  [primary_parser 'f(a,a*b) = b'; primary_parser 'g(a*b,b) = a'; primary_parser '1 * a = a'; primary_parser 'a * 1 = a'];;
...
let eqs =  [primary_parser 'f(a,f(b,c,a),d) = c'];;
...
let eqs =  [primary_parser 'f(a,f(b,c,a),d) = c'; primary_parser 'f(a,b,c) = g(a,b)';
                     primary_parser 'g(a,b) = h(b)'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z'; primary_parser '1 * x = x'; primary_parser 'i(x) * x = 1'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z'; primary_parser 'x * 1 = x'; primary_parser 'x * i(x) = 1'];;
...
let eqs =  [primary_parser 'i(a) * (a * b) = b'];;

complete_and_simplify ["1"; "*"; "i"] eqs;;

let eqs =  [primary_parser 'a * (i(a) * b) = b'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z';
  primary_parser '1 * x = x'; primary_parser '11 * x = x';
  primary_parser 'i(x) * x = 1'; primary_parser 'j(x) * x = 11'];;
...
let eqs =  [primary_parser '(a * b) * (c * b * a) = b'];;
...
let eqs =  [primary_parser 'f(a,a*b) = b'; primary_parser 'g(a*b,b) = a'];;
...
let eqs =
  [primary_parser '(x * y) * z = x * y * z';
   primary_parser 'f(a,a*b) = b'; primary_parser 'g(a*b,b) = a'];;
...
let eqs =
 [primary_parser 'a * \(a,b) = b'; primary_parser '/(a,b) * b = a'; primary_parser '1 * a = a'; primary_parser 'a * 1 = a'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z';
  primary_parser '1 * 1 = 1';
  primary_parser 'a * i(a) = 1';
  primary_parser 'f(1,a,b) = a';
  primary_parser 'f(a*b,a,b) = g(a*b,b)'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z'; primary_parser 'x * 1 = x'; primary_parser 'i(x) * x = 1'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z';
  primary_parser '1 * x = x'; primary_parser '11 * x = x';
  primary_parser 'x * i(x) = 1'; primary_parser 'x * j(x) = 11'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z';
  primary_parser '1 * x = x';
  primary_parser 'prime(a) * a = star(a)';
  primary_parser 'star(a) * b = b'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z';
  primary_parser '1 * x = x';
  primary_parser 'hash(a) * dollar(a) * a = star(a)';
  primary_parser 'star(a) * b = b';
  primary_parser 'a * hash(a) = 1';
  primary_parser 'a * 1 = hash(hash(a))';
  primary_parser 'hash(hash(hash(a))) = hash(a)'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * y * z';
  ...
  primary_parser 'star(a) * b = b';
  primary_parser 'a * hash(a) = 1';
  primary_parser 'hash(hash(a)) = a * 1';
  primary_parser 'hash(hash(hash(a))) = hash(a)'];;
...
let eqs =
 [primary_parser '(a * a) * a = one(a)';
  primary_parser 'a * (a * a) = two(a)';
  primary_parser '(a * b) * (b * c) = b';
  primary_parser 'two(a) * b = a * b'];;
...
let eqs =  [primary_parser 'f(f(f(f(f(1))))) = 1'; primary_parser 'f(f(f(1))) = 1'];;
...
let eqs =
 [primary_parser 'x * i(y * (((z * i(z)) * i(u * y)) * x)) = u'];;

complete_and_simplify ["1"; "*"; "i"] eqs;;

let eqs =
 [primary_parser '((1 / (x / (y / (((x / x) / x) / z)))) / z) = y'];;

complete_and_simplify ["1"; "/"] eqs;;

let eqs =
 [primary_parser 'i(x * i(x)) * (i(i((y * z) * u) * y) * i(u)) = z'];;
...
let eqs =  [primary_parser 'f(f(x)) = g(x)'];;
...
let eqs =
 [primary_parser '(x * y) * z = x * (y * z)'; primary_parser '1 * x = x'; primary_parser 'x * 1 = x'; primary_parser 'x * x = 1']
and wts = ["1"; "*"; "i"];;
...
(*
let eqs =
 [primary_parser 'f(f(x)) = f(x)';
  primary_parser 'g(g(x)) = f(x)';
  primary_parser 'f(g(x)) = g(x)';
  primary_parser 'g(f(x)) = f(x)'];;

complete_and_simplify ["f"; "g"] eqs;;

let eqs =  [primary_parser 'f(g(f(x))) = g(x)'];;
...
let eqs =
 [primary_parser '0 + y = y';
  primary_parser 'SUC(x) + y = SUC(x + y)';
  primary_parser 'append(nil,l) = l';
  primary_parser 'append(h::t,l) = h::append(t,l)';
  primary_parser 'length(nil) = 0';
  primary_parser 'length(h::t) = SUC(length(t))';
  primary_parser 'rev(nil) = nil';
  primary_parser 'rev(h::t) = append(rev(t),h::nil)'];;
...
iprove [] primary_parser 'x + 0 = x';;

iprove [] primary_parser 'x + SUC(y) = SUC(x + y)';;

iprove [] primary_parser '(x + y) + z = x + y + z';;

iprove [] primary_parser 'length(append(x,y)) = length(x) + length(y)';;

iprove [] primary_parser 'append(append(x,y),z) = append(x,append(y,z))';;

iprove [] primary_parser 'append(x,nil) = x';;

iprove [primary_parser 'append(append(x,y),z) = append(x,append(y,z))';
        primary_parser 'append(x,nil) = x']
        primary_parser 'rev(append(x,y)) = append(rev(y),rev(x))';;

iprove [primary_parser 'rev(append(x,y)) = append(rev(y),rev(x))';
        primary_parser 'append(x,nil) = x';
        primary_parser 'append(append(x,y),z) = append(x,append(y,z))']
        primary_parser 'rev(rev(x)) = x';;

(* ------------------------------------------------------------------------- *)
(* Here it's not immediately so obvious since we get extra equs.             *)
(* ------------------------------------------------------------------------- *)

iprove [] primary_parser 'rev(rev(x)) = x';;
...
iprove [] primary_parser 'length(append(x,y)) = length(x)';; (*** try something false ***)
...
(***
meson primary_parser 'forall x. p(x)';;
tab primary_parser 'forall x. p(x)';;
 ***)
...
(***
resolution primary_parser 'forall x. p(x)';;
 ***)
...
(*
let fm = primary_parser '(forall x. p(x)) \/ (exists y. p(y))';;
...
aedecide primary_parser 'exists y. forall x. P(y) ==> P(x)';;

davisputnam primary_parser 'exists y. forall x. P(y) ==> P(x)';;
...
(*
miniscope(nnf primary_parser 'exists y. forall x. P(y) ==> P(x)');;
...
let all_possible_syllogisms' =
  let p =
    primary_parser '(exists x. P(x)) /\ (exists x. M(x)) /\ (exists x. S(x))' in
...
(*
decide_fmp
 primary_parser '(forall x y. R(x,y) \/ R(y,x)) ==> forall x. R(x,x)';;

decide_fmp
 primary_parser '(forall x y z. R(x,y) /\ R(y,z) ==> R(x,z)) ==> forall x. R(x,x)';;
...
(*
quelim_dlo primary_parser 'forall x y. exists z. z < x /\ z < y';;

quelim_dlo primary_parser 'exists z. z < x /\ z < y';;

quelim_dlo primary_parser 'exists z. x < z /\ z < y';;

quelim_dlo primary_parser '(forall x. x < a ==> x < b)';;

quelim_dlo primary_parser 'forall a b. (forall x. x < a ==> x < b) <=> a <= b';;
...
time quelim_dlo primary_parser 'forall x. exists y. x < y';;

time quelim_dlo primary_parser 'forall x y z. x < y /\ y < z ==> x < z';;

time quelim_dlo primary_parser 'forall x y. x < y \/ (x = y) \/ y < x';;

time quelim_dlo primary_parser 'exists x y. x < y /\ y < x';;

time quelim_dlo primary_parser 'forall x y. exists z. z < x /\ x < y';;

time quelim_dlo primary_parser 'exists z. z < x /\ x < y';;

time quelim_dlo primary_parser 'forall x y. exists z. z < x /\ z < y';;

time quelim_dlo primary_parser 'forall x y. x < y ==> exists z. x < z /\ z < y';;

time quelim_dlo
  primary_parser 'forall x y. ~(x = y) ==> exists u. u < x /\ (y < u \/ x < y)';;

time quelim_dlo primary_parser 'exists x. x = x';;

time quelim_dlo primary_parser 'exists x. x = x /\ x = y';;

time quelim_dlo primary_parser 'exists z. x < z /\ z < y';;

time quelim_dlo primary_parser 'exists z. x <= z /\ z <= y';;

time quelim_dlo primary_parser 'exists z. x < z /\ z <= y';;

time quelim_dlo primary_parser 'forall x y z. exists u. u < x /\ u < y /\ u < z';;

time quelim_dlo primary_parser 'forall y. x < y /\ y < z ==> w < z';;

time quelim_dlo primary_parser 'forall x y. x < y';;

time quelim_dlo primary_parser 'exists z. z < x /\ x < y';;

time quelim_dlo primary_parser 'forall a b. (forall x. x < a ==> x < b) <=> a <= b';;

time quelim_dlo primary_parser 'forall x. x < a ==> x < b';;

time quelim_dlo primary_parser 'forall x. x < a ==> x <= b';;

time quelim_dlo primary_parser 'forall a b. exists x. ~(x = a) \/ ~(x = b) \/ (a = b)';;

time quelim_dlo primary_parser 'forall x y. x <= y \/ x > y';;
...
(*
integer_qelim primary_parser 'forall x y. ~(2 * x + 1 = 2 * y)';;

integer_qelim primary_parser 'forall x. exists y. 2 * y <= x /\ x < 2 * (y + 1)';;

integer_qelim primary_parser 'exists x y. 4 * x - 6 * y = 1';;

integer_qelim <<forall x. ~divides(2,x) /\ divides(3,x-1) <=>
                          divides(12,x-1) \/ divides(12,x-7)>>;;

integer_qelim primary_parser 'forall x. b < x ==> a <= x';;
*)
...
(*
natural_qelim primary_parser 'forall d. exists x y. 3 * x + 5 * y = d';;
integer_qelim primary_parser 'forall d. exists x y. 3 * x + 5 * y = d';;
natural_qelim primary_parser 'forall d. d >= 8 ==> exists x y. 3 * x + 5 * y = d';;
natural_qelim primary_parser 'forall d. exists x y. 3 * x - 5 * y = d';;
...
integer_qelim primary_parser 'exists x y. x > 0 /\ y >= 0 /\ 3 * x - 5 * y = 1';;

integer_qelim primary_parser 'exists x y z. 4 * x - 6 * y = 1';;

integer_qelim primary_parser 'forall x. a < 3 * x ==> b < 3 * x';;

time integer_qelim primary_parser 'forall x y. x <= y ==> 2 * x + 1 < 2 * y';;

time integer_qelim primary_parser '(exists d. y = 65 * d) ==> (exists d. y = 5 * d)';;

time integer_qelim
  primary_parser 'forall y. (exists d. y = 65 * d) ==> (exists d. y = 5 * d)';;

time integer_qelim primary_parser 'forall x y. ~(2 * x + 1 = 2 * y)';;

time integer_qelim
  primary_parser 'forall x y z. (2 * x + 1 = 2 * y) ==> x + y + z > 129';;

time integer_qelim primary_parser 'forall x. a < x ==> b < x';;
...
time integer_qelim primary_parser 'forall a b. exists x. a < 20 * x /\ 20 * x < b';;

time integer_qelim primary_parser 'exists x. a < 20 * x /\ 20 * x < b';;

time integer_qelim primary_parser 'forall b. exists x. a < 20 * x /\ 20 * x < b';;

time integer_qelim
  primary_parser 'forall a. exists b. a < 4 * b + 3 * a \/ (~(a < b) /\ a > b + 1)';;

time integer_qelim
  primary_parser 'exists y. forall x. x + 5 * y > 1 /\ 13 * x - y > 1 /\ x + 2 < 0';;
...
time integer_qelim primary_parser 'exists x y. 5 * x + 3 * y = 1';;

time integer_qelim primary_parser 'exists x y. 5 * x + 10 * y = 1';;

time integer_qelim primary_parser 'exists x y. x >= 0 /\ y >= 0 /\ 5 * x - 6 * y = 1';;


time integer_qelim primary_parser 'exists w x y z. 2 * w + 3 * x + 4 * y + 5 * z = 1';;

time integer_qelim primary_parser 'exists x y. x >= 0 /\ y >= 0 /\ 5 * x - 3 * y = 1';;

time integer_qelim primary_parser 'exists x y. x >= 0 /\ y >= 0 /\ 3 * x - 5 * y = 1';;

time integer_qelim primary_parser 'exists x y. x >= 0 /\ y >= 0 /\ 6 * x - 3 * y = 1';;

time integer_qelim
  primary_parser 'forall x y. ~(x = 0) ==> 5 * y < 6 * x \/ 5 * y > 6 * x';;

time integer_qelim
  primary_parser 'forall x y. ~divides(5,x) /\ ~divides(6,y) ==> ~(6 * x = 5 * y)';;

time integer_qelim primary_parser 'forall x y. ~divides(5,x) ==> ~(6 * x = 5 * y)';;

time integer_qelim primary_parser 'forall x y. ~(6 * x = 5 * y)';;

time integer_qelim primary_parser 'forall x y. 6 * x = 5 * y ==> exists d. y = 3 * d';;

time integer_qelim primary_parser '6 * x = 5 * y ==> exists d. y = 3 * d';;
...
time integer_qelim
  primary_parser 'forall z. z > 7 ==> exists x y. x >= 0 /\ y >= 0 /\ 3 * x + 5 * y = z';;

time integer_qelim
  primary_parser 'forall z. z > 2 ==> exists x y. x >= 0 /\ y >= 0 /\ 3 * x + 5 * y = z';;
...
(integer_qelim ** generalize)
  primary_parser 'a + 2 = b /\ v_3 = b - a + 1 /\ v_2 = b - 2 /\ v_1 = 3 ==> false';;
...
integer_qelim
  primary_parser 'exists a b. ~(a = 1) /\ ((2 * b = a) \/ (2 * b = 3 * a + 1)) /\ (a = b)';;
...
integer_qelim
  primary_parser 'forall x. x >= 8 ==> exists u v. u >= 0 /\ v >= 0 /\ x = 3 * u + 5 * v';;
...
(*
complex_qelim
 primary_parser 'forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 1 = 0';;

complex_qelim
 primary_parser 'forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + c = 0';;
...
time complex_qelim primary_parser 'exists x. x + 2 = 3';;

time complex_qelim primary_parser 'exists x. x^2 + a = 3';;

time complex_qelim primary_parser 'exists x. x^2 + x + 1 = 0';;

time complex_qelim primary_parser 'exists x. x^2 + x + 1 = 0 /\ x^3 + x^2 + 1 = 0';;

time complex_qelim primary_parser 'exists x. x^2 + 1 = 0 /\ x^4 + x^3 + x^2 + x = 0';;

time complex_qelim primary_parser 'forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 1 = 0';;

time complex_qelim primary_parser 'forall a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 2 = 0';;

time complex_qelim primary_parser 'exists a x. a^2 = 2 /\ x^2 + a * x + 1 = 0 /\ ~(x^4 + 2 = 0)';;

time complex_qelim primary_parser 'exists x. a^2 = 2 /\ x^2 + a * x + 1 = 0 /\ ~(x^4 + 2 = 0)';;

time complex_qelim primary_parser 'forall x. x^2 + a * x + 1 = 0 ==> x^4 + 2 = 0';;

time complex_qelim primary_parser 'forall a. a^2 = 2 /\ x^2 + a * x + 1 = 0 ==> x^4 + 2 = 0';;
...
time complex_qelim primary_parser 'forall x. exists y. x^2 = y^3';;
...
time complex_qelim
 primary_parser 'forall a b. ~(a = b) ==> exists x y. (y * x^2 = a) /\ (y * x^2 + x = b)';;
...
time complex_qelim
  primary_parser 'forall x y. x^2 + y^2 = 1 ==> (2 * y^2 - 1)^2 + (2 * x * y)^2 = 1';;
...
time complex_qelim primary_parser 'exists z. x * z^87 + y * z^44 + 1 = 0';;

time complex_qelim primary_parser 'forall u. exists v. x * (u + v^2)^2 + y * (u + v^2) + z = 0';;
...
(*
real_qelim primary_parser 'exists x. x^4 + x^2 + 1 = 0';;

real_qelim primary_parser 'exists x. x^3 - x^2 + x - 1 = 0';;

real_qelim <<exists x y. x^3 - x^2 + x - 1 = 0 /\
                         y^3 - y^2 + y - 1 = 0 /\ ~(x = y)>>;;

#trace testform;;
real_qelim primary_parser 'exists x. x^2 - 3 * x + 2 = 0 /\ 2 * x - 3 = 0';;
#untrace testform;;

real_qelim
 primary_parser 'forall a f k. (forall e. k < e ==> f < a * e) ==> f <= a * k';;

real_qelim primary_parser 'exists x. a * x^2 + b * x + c = 0';;
...
(*
grobner_decide
  primary_parser 'a^2 = 2 /\ x^2 + a*x + 1 = 0 ==> x^4 + 1 = 0';;

grobner_decide
  primary_parser 'a^2 = 2 /\ x^2 + a*x + 1 = 0 ==> x^4 + 2 = 0';;
...
time grobner_decide  primary_parser 'a^2 = 2 /\ x^2 + a*x + 1 = 0 ==> x^4 + 1 = 0';;

time grobner_decide  primary_parser 'a^2 = 2 /\ x^2 + a*x + 1 = 0 ==> x^4 + 2 = 0';;
...
time grobner_decide
 primary_parser 'y * (c * x + d) = a * x + b ==> x * (c * y - a) = b - d * y';;
...
time grobner_decide
 primary_parser '(y - x) * s^2 = 1 /\ (x - y) * t^2 = 1 ==> s^2 + t^2 = 0';;
...
let coordinations =
  ["collinear", (** Points 1, 2 and 3 lie on a common line **)
   primary_parser '(1_x - 2_x) * (2_y - 3_y) = (1_y - 2_y) * (2_x - 3_x)';
   "parallel", (** Lines (1,2) and (3,4) are parallel **)
    primary_parser '(1_x - 2_x) * (3_y - 4_y) = (1_y - 2_y) * (3_x - 4_x)';
   "perpendicular", (** Lines (1,2) and (3,4) are perpendicular **)
   primary_parser '(1_x - 2_x) * (3_x - 4_x) + (1_y - 2_y) * (3_y - 4_y) = 0';
   "lengths_eq", (** Lines (1,2) and (3,4) have the same length **)
   primary_parser '(1_x - 2_x)^2 + (1_y - 2_y)^2 = (3_x - 4_x)^2 + (3_y - 4_y)^2';
   "is_midpoint", (** Point 1 is the midpoint of line (2,3) **)
   primary_parser '2 * 1_x = 2_x + 3_x /\ 2 * 1_y = 2_y + 3_y';
   ...
   primary_parser '(1_x = 2_x) /\ (1_y = 2_y)'];;
...
(*
coordinate primary_parser 'collinear(a,b,c) ==> collinear(b,a,c)';;
*)
...
(*
real_qelim
 primary_parser 'forall x y. exists s c. s^2 + c^2 = 1 /\ s * x + c * y = 0';;
*)
...
let invariant_under_scaling fm =
  Imp(primary_parser '~(A = 0)',invariant(secondary_parser 'A * x|',secondary_parser 'A * y|') fm);;
...
(*
let p = prenex
 primary_parser '(forall x. R(x,f(x))) /\ (forall x y. S(x,y) <=> R(x,y) \/ R(y,x))'
and q = prenex
 primary_parser '(forall x y z. S(x,y) /\ S(y,z) ==> T(x,z)) /\ ~T(0,0)';;
...
(*
let p = primary_parser '(p ==> q /\ r)'
and q = primary_parser '~((q ==> p) ==> s ==> (p <=> q))';;
...
and q = primary_parser '(forall x y z. S(x,y) /\ S(y,z) ==> T(x,z)) /\ ~T(u,u)';;
...
test_interp primary_parser 'forall x. P(x) ==> exists y. forall z. P(z) ==> Q(y)';;
...
let p = primary_parser 'forall x. L(x,b)'
and q = primary_parser '(forall y. L(b,y) ==> m = y) /\ ~(m = b)';;
...
let p =
 primary_parser '(forall x. A(x) /\ C(x) ==> B(x)) /\ (forall x. D(x) \/ ~D(x) ==> C(x))'
and q =
 primary_parser '~(forall x. E(x) ==> A(x) ==> B(x))';;
...
ccvalid
  primary_parser '(v_2 = f(v_3) /\ v_1 = f(u)) ==> ~(u = v_3 /\ ~(v_1 = v_2))';;
*)
...
nelop (add_default [int_lang])
 primary_parser 'f(v - 1) - 1 = v + 1 /\ f(u) + 1 = u - 1 /\ u + 1 = v ==> false';;
...
(*
nelop (add_default [int_lang])
 primary_parser 'y <= x /\ y >= x + z /\ z >= 0 ==> f(f(x) - f(y)) = f(z)';;

nelop (add_default [int_lang])
 primary_parser 'x = y /\ y >= z /\ z >= x ==> f(z) = f(x)';;
...
map (real_qelim ** generalize)
  [primary_parser 'x * y = 0 /\ z = 0 ==> x = z \/ y = z';
   primary_parser 'x * y = 0 /\ z = 0 ==> x = z';
   primary_parser 'x * y = 0 /\ z = 0 ==> y = z'];;

map (integer_qelim ** generalize)
  [primary_parser '0 <= x /\ x < 2 /\ y = 0 /\ z = 1 ==> x = y \/ x = z';
   primary_parser '0 <= x /\ x < 2 /\ y = 0 /\ z = 1 ==> x = y';
   primary_parser '0 <= x /\ x < 2 /\ y = 0 /\ z = 1 ==> x = z'];;
...
nelop (add_default [int_lang])
 primary_parser 'f(v) = v /\ f(u) = u - 1 /\ u = v ==> false';;
...
time (nelop (add_default [int_lang]))
 primary_parser 'z = f(x - y) /\ x = z + y /\ ~(-(y) = -(x - f(f(z)))) ==> false';;

(*** This (ICS theories-1) fails without array operations ***)

time (nelop (add_default [int_lang]))
 primary_parser 'a + 2 = b ==> f(read(update(A,a,3),b-2)) = f(b - a + 1)';;

(*** can-001 from ICS examples site, with if-then-elses expanded manually ***)

time (nelop (add_default [int_lang]))
 primary_parser '(x = y /\ z = 1 ==> f(f((x+z))) = f(f((1+y))))';;
...
time (nelop (add_default [int_lang]))
 primary_parser 'x <= g(x) /\ x >= g(x) ==> x = g(g(g(g(x))))';;

(*** Easy example I invented ***)

time (nelop (add_default [real_lang]))
 primary_parser 'x^2 =  1 ==> (f(x) = f(-(x)))  ==> (f(x) = f(1))';;

(*** Taken from Clark Barrett's CVC page ***)

time (nelop (add_default [int_lang]))
 primary_parser '2 * f(x + y) = 3 * y /\ 2 * x = y ==> f(f(x + y)) = 3 * x';;
...
(*
lcftaut primary_parser '(p ==> q) \/ (q ==> p)';;

lcftaut primary_parser 'p /\ q <=> ((p <=> q) <=> p \/ q)';;

lcftaut primary_parser '((p <=> q) <=> r) <=> (p <=> (q <=> r))';;
*)
(* ========================================================================= *)
...
(* ------------------------------------------------------------------------- *)
(*                                                                           *)
(* -------------------------------------------- alpha "z" primary_parser 'forall x. p[x]' *)
...
(* ------------------------------------------------------------------------- *)
(*                                                                           *)
(* -------------------------------- ispec t primary_parser 'forall x. p[x]'               *)
(*   |- (forall x. p[x]) ==> p'[t]                                           *)
(* ------------------------------------------------------------------------- *)
...
(*
ispec secondary_parser 'y|' primary_parser 'forall x y z. x + y + z = z + y + x';;
...
isubst secondary_parser 'x + x|' secondary_parser '2 * x|'
        primary_parser 'x + x = x ==> x = 0' primary_parser '2 * x = x ==> x = 0';;

isubst secondary_parser 'x + x|'  secondary_parser '2 * x|'
       primary_parser '(x + x = y + y) ==> (y + y + y = x + x + x)'
       primary_parser '2 * x = y + y ==> y + y + y = x + 2 * x';;

ispec secondary_parser 'x|' primary_parser 'forall x y z. x + y + z = y + z + z' ;;

ispec secondary_parser 'x|' primary_parser 'forall x. x = x' ;;

ispec secondary_parser 'w + y + z|' primary_parser 'forall x y z. x + y + z = y + z + z' ;;

ispec secondary_parser 'x + y + z|' primary_parser 'forall x y z. x + y + z = y + z + z' ;;

ispec secondary_parser 'x + y + z|' primary_parser 'forall x y z. nothing_much' ;;

isubst secondary_parser 'x + x|' secondary_parser '2 * x|'
       primary_parser '(x + x = y + y) <=> (something \/ y + y + y = x + x + x)' ;;

isubst secondary_parser 'x + x|'  secondary_parser '2 * x|'
       primary_parser '(exists x. x = 2) <=> exists y. y + x + x = y + y + y'
       primary_parser '(exists x. x = 2) <=> (exists y. y + 2 * x = y + y + y)';;

isubst secondary_parser 'x|'  secondary_parser 'y|'
        primary_parser '(forall z. x = z) <=> (exists x. y < z) /\ (forall y. y < x)'
        primary_parser '(forall z. y = z) <=> (exists x. y < z) /\ (forall y'. y' < y)';;

(* ------------------------------------------------------------------------- *)
(* The bug is now fixed.                                                     *)
(* ------------------------------------------------------------------------- *)

ispec secondary_parser 'x'|' primary_parser 'forall x x' x''. x + x' + x'' = 0';;

ispec secondary_parser 'x''|' primary_parser 'forall x x' x''. x + x' + x'' = 0';;

ispec secondary_parser 'x' + x''|' primary_parser 'forall x x' x''. x + x' + x'' = 0';;

ispec secondary_parser 'x + x' + x''|' primary_parser 'forall x x' x''. x + x' + x'' = 0';;

ispec secondary_parser '2 * x|' primary_parser 'forall x x'. x + x' = x' + x';;
...
(* ------------------------------------------------------------------------- *)
(*    |- (q ==> f) ==> ... ==> (q ==> p) ==> r                               *)
(* --------------------------------------------- use_laterimp primary_parser 'q ==> p'    *)
(*    |- (p ==> f) ==> ... ==> (q ==> p) ==> r                               *)
(* ------------------------------------------------------------------------- *)
...
lcfrefute primary_parser 'p(1) /\ ~q(1) /\ (forall x. p(x) ==> q(x))' 1 simpcont;;

lcfrefute primary_parser '(exists x. ~p(x)) /\ (forall x. p(x))' 1 simpcont;;
...
let ewd954 = prove
 <<(forall x y. x <= y <=> x * y = x) /\
   ...
   ==> forall x y. x <= y ==> f(x) <= f(y)>>
 [note("eq_sym",primary_parser 'forall x y. x = y ==> y = x')
    using [eq_sym secondary_parser 'x|' secondary_parser 'y|'];
  note("eq_trans",primary_parser 'forall x y z. x = y /\ y = z ==> x = z')
    using [eq_trans secondary_parser 'x|' secondary_parser 'y|' secondary_parser 'z|'];
  note("eq_cong",primary_parser 'forall x y. x = y ==> f(x) = f(y)')
    using [axiom_funcong "f" [secondary_parser 'x|'] [secondary_parser 'y|']];
  assume ["le",primary_parser 'forall x y. x <= y <=> x * y = x';
          "hom",primary_parser 'forall x y. f(x * y) = f(x) * f(y)'];
  fix "x"; fix "y";
  assume ["xy",primary_parser 'x <= y'];
  so have primary_parser 'x * y = x' by ["le"];
  so have primary_parser 'f(x * y) = f(x)' by ["eq_cong"];
  so have primary_parser 'f(x) = f(x * y)' by ["eq_sym"];
  so have primary_parser 'f(x) = f(x) * f(y)' by ["eq_trans"; "hom"];
  so have primary_parser 'f(x) * f(y) = f(x)' by ["eq_sym"];
  so conclude primary_parser 'f(x) <= f(y)' by ["le"];
  qed];;
*)
...
prove
 <<(exists x. p(x)) ==> (forall x. p(x) ==> p(f(x)))
  ...
  [assume ["A",primary_parser 'exists x. p(x)'];
   assume ["B",primary_parser 'forall x. p(x) ==> p(f(x))'];
   note ("C",primary_parser 'forall x. p(x) ==> p(f(f(f(f(x)))))')
   proof
    [have primary_parser 'forall x. p(x) ==> p(f(f(x)))' by ["B"];
     so conclude primary_parser 'forall x. p(x) ==> p(f(f(f(f(x)))))' at once;
     qed];
   consider ("a",primary_parser 'p(a)') by ["A"];
   take secondary_parser 'a|';
   so conclude primary_parser 'p(f(f(f(f(a)))))' by ["C"];
   qed];;
...
prove
 <<(exists x. p(x)) ==> (forall x. p(x) ==> p(f(x)))
  ...
  [assume ["A",primary_parser 'exists x. p(x)'];
   assume ["B",primary_parser 'forall x. p(x) ==> p(f(x))'];
   lemma ("C",primary_parser 'forall x. p(x) ==> p(f(f(f(f(x)))))');
     have primary_parser 'forall x. p(x) ==> p(f(f(x)))' by ["B"];
     so conclude primary_parser 'forall x. p(x) ==> p(f(f(f(f(x)))))' at once;
     qed;
   consider ("a",primary_parser 'p(a)') by ["A"];
...
prove
 <<(exists x. p(x)) ==> (forall x. p(x) ==> p(f(x)))
  ...
  [assume ["A",primary_parser 'exists x. p(x)'];
   assume ["B",primary_parser 'forall x. p(x) ==> p(f(x))'];
   note ("C",primary_parser 'forall x. p(x) ==> p(f(f(f(f(x)))))') proof
    [have primary_parser 'forall x. p(x) ==> p(f(f(x)))' by ["B"];
...
prove <<forall a. p(a) ==> (forall x. p(x) ==> p(f(x)))
                  ==> exists y. p(y) /\ p(f(y))>>
      [fix "c";
       assume ["A",primary_parser 'p(c)'];
       assume ["B",primary_parser 'forall x. p(x) ==> p(f(x))'];
       take secondary_parser 'c|';
       conclude primary_parser 'p(c)' by ["A"];
       note ("C",primary_parser 'p(c) ==> p(f(c))') by ["B"];
...
prove <<forall a. p(a) ==> (forall x. p(x) ==> p(f(x)))
      ...
      [fix "c";
       ...
       assume ["B",primary_parser 'forall x. p(x) ==> p(f(x))'];
       take secondary_parser 'c|';
       note ("D",primary_parser 'p(c)') by ["A"];
       note ("C",primary_parser 'p(c) ==> p(f(c))') by ["B"];
       our thesis by ["C"; "A"; "D"];
       qed];;


prove primary_parser '(p(a) \/ p(b)) ==> q ==> exists y. p(y)'
  [assume ["A",primary_parser 'p(a) \/ p(b)'];
   assume ["",primary_parser 'q'];
   cases primary_parser 'p(a) \/ p(b)' by ["A"];
     take secondary_parser 'a|';
     so our thesis at once;
     qed;
...
prove
  primary_parser '(p(a) \/ p(b)) /\ (forall x. p(x) ==> p(f(x))) ==> exists y. p(f(y))'
  [assume ["base",primary_parser 'p(a) \/ p(b)';
           "Step",primary_parser 'forall x. p(x) ==> p(f(x))'];
   cases primary_parser 'p(a) \/ p(b)' by ["base"];
     so note("A",primary_parser 'p(a)') at once;
     note ("X",primary_parser 'p(a) ==> p(f(a))') by ["Step"];
     take secondary_parser 'a|';
     our thesis by ["A"; "X"];
     qed;
...
prove
 primary_parser '(exists x. p(x)) ==> (forall x. p(x) ==> p(f(x))) ==> exists y. p(f(y))'
  [assume ["A",primary_parser 'exists x. p(x)'];
   assume ["B",primary_parser 'forall x. p(x) ==> p(f(x))'];
   consider ("a",primary_parser 'p(a)') by ["A"];
   so note ("concl",primary_parser 'p(f(a))') by ["B"];
   take secondary_parser 'a|';
   our thesis by ["concl"];
   qed];;
...
prove <<(forall x. p(x) ==> q(x)) ==> (forall x. q(x) ==> p(x))
       ==> (p(a) <=> q(a))>>
  [assume ["A",primary_parser 'forall x. p(x) ==> q(x)'];
   assume ["B",primary_parser 'forall x. q(x) ==> p(x)'];
   note ("von",primary_parser 'p(a) ==> q(a)') by ["A"];
   note ("bis",primary_parser 'q(a) ==> p(a)') by ["B"];
...
prove
  primary_parser '(p(a) \/ p(b)) /\ (forall x. p(x) ==> p(f(x))) ==> exists y. p(f(y))'
  [assume ["A",primary_parser 'antecedent'];
   note ("Step",primary_parser 'forall x. p(x) ==> p(f(x))') by ["A"];
   per_cases by ["A"];
     suppose ("base",primary_parser 'p(a)');
     note ("X",primary_parser 'p(a) ==> p(f(a))') by ["Step"];
     take secondary_parser 'a|';
     our thesis by ["base"; "X"];
     qed;

     suppose ("base",primary_parser 'p(b)');
...
let testcase n =
  gen "x" (funpow n double_th (lcftaut primary_parser 'p(x) ==> q(1) \/ p(x)'));;
...
(*
gform primary_parser 'x = x';;
gform primary_parser '0 < 0';;
*)
...
let robinson_thm =
  ...
  [note("eq_refl",primary_parser 'forall x. x = x') using [axiom_eqrefl (Var "x")];
   note("eq_trans",primary_parser 'forall x y z. x = y ==> y = z ==> x = z')
      using [eq_trans (Var "x") (Var "y") (Var "z")];
   note("eq_sym",primary_parser 'forall x y. x = y ==> y = x')
      using [eq_sym (Var "x") (Var "y")];
   note("suc_cong",primary_parser 'forall a b. a = b ==> S(a) = S(b)')
      using [axiom_funcong "S" [Var "a"] [Var "b"]];
   note("add_cong",
        primary_parser 'forall a b c d. a = b /\ c = d ==> a + c = b + d')
   ...
   assume ["suc_inj",primary_parser 'forall m n. S(m) = S(n) ==> m = n';
           "num_nz",primary_parser 'forall n. ~(n = 0) <=> exists m. n = S(m)';
           "add_0",primary_parser 'forall n. 0 + n = n';
           "add_suc",primary_parser 'forall m n. S(m) + n = S(m + n)';
           "mul_0",primary_parser 'forall n. 0 * n = 0';
           "mul_suc",primary_parser 'forall m n. S(m) * n = n + m * n';
           "le_def",primary_parser 'forall m n. m <= n <=> exists d. m + d = n';
           "lt_def",primary_parser 'forall m n. m < n <=> S(m) <= n'];
   note("not_suc_0",primary_parser 'forall n. ~(S(n) = 0)') by ["num_nz"; "eq_refl"];
   so conclude primary_parser 'forall n. S(n) = 0 ==> false' at once;
   so conclude primary_parser 'forall n. 0 = S(n) ==> false' by ["eq_sym"];
   note("num_cases",primary_parser 'forall n. (n = 0) \/ exists m. n = S(m)')
         by ["num_nz"];
   note("suc_inj_eq",primary_parser 'forall m n. S(m) = S(n) <=> m = n')
     by ["suc_inj"; "suc_cong"];
   so conclude
     primary_parser 'forall m n. (m = n ==> false) ==> (S(m) = S(n) ==> false)'
     at once;
   conclude primary_parser 'forall m n. (exists d. m + d = n) ==> m <= n'
     by ["le_def"];
   conclude primary_parser 'forall m n. S(m) <= n ==> m < n' by ["lt_def"];
   ...
   conclude <<forall m n. (forall d. d < n ==> d = m ==> false)
                          ==> m < n ==> false>>
     by ["eq_refl"; "lt_cong"];
   have primary_parser '0 <= 0' by ["le_def"; "add_0"];
   so have primary_parser 'forall x. x = 0 ==> x <= 0'
     by ["le_cong"; "eq_refl"; "eq_sym"];
   so conclude primary_parser 'forall n. n <= 0 \/ (exists m. S(m) = n)'
     by ["num_nz"; "eq_sym"];
   note("add_eq_0",primary_parser 'forall m n. m + n = 0 ==> m = 0 /\ n = 0') proof
    [fix "m"; fix "n";
     assume ["A",primary_parser 'm + n = 0'];
     cases primary_parser 'm = 0 \/ exists p. m = S(p)' by ["num_cases"];
       so conclude primary_parser 'm = 0' at once;
       so have primary_parser 'm + n = 0 + n' by ["add_cong"; "eq_refl"];
       so our thesis by ["A"; "add_0"; "eq_sym"; "eq_trans"];
     qed;
       so consider ("p",primary_parser 'm = S(p)') at once;
       so have primary_parser 'm + n = S(p) + n' by ["add_cong"; "eq_refl"];
       so have primary_parser 'm + n = S(p + n)' by ["eq_trans"; "add_suc"];
       so have primary_parser 'S(p + n) = 0' by ["A"; "eq_sym"; "eq_trans"];
       so our thesis by ["not_suc_0"];
     qed];
   so conclude primary_parser 'forall n. n <= 0 ==> n = 0' by ["le_def"];
   have primary_parser 'forall m n. S(m) <= S(n) ==> m <= n' proof
    [fix "m"; fix "n";
     assume ["lesuc",primary_parser 'S(m) <= S(n)'];
     so consider("d",primary_parser 'S(m) + d = S(n)') by ["le_def"];
     so have primary_parser 'S(m + d) = S(n)' by ["add_suc"; "eq_sym"; "eq_trans"];
     so have primary_parser 'm + d = n' by ["suc_inj"];
     so conclude primary_parser 'm <= n' by ["le_def"];
     qed];
   so conclude primary_parser 'forall m n. S(m) <= S(n) ==> m <= n' at once;
   so conclude primary_parser 'forall m n. m < S(n) ==> m <= n' by ["lt_def"];
   fix "n";
   assume ["hyp",primary_parser 'n < 0'];
   so have primary_parser 'S(n) <= 0' by ["lt_def"];
   so consider("d",primary_parser 'S(n) + d = 0') by ["le_def"];
   so have primary_parser 'S(n + d) = 0' by ["add_suc"; "eq_trans"; "eq_sym"];
   so our thesis by ["not_suc_0"];
   qed];;
 ...
 <<(forall p. |--(p) ==> |--(Pr(p))) /\
   ...
   ==> (forall p q. |--(imp(p,q)) /\ |--(p) ==> |--(q)) /\
       ...
       ==> |--(imp(G,imp(Pr(G),F))) /\ |--(imp(imp(Pr(G),F),G))
           ==> |--(imp(Pr(F),F)) ==> |--(F)>>
 [assume["lob1",primary_parser 'forall p. |--(p) ==> |--(Pr(p))';
         "lob2",primary_parser 'forall p q. |--(imp(Pr(imp(p,q)),imp(Pr(p),Pr(q))))';
         "lob3",primary_parser 'forall p. |--(imp(Pr(p),Pr(Pr(p))))'];
